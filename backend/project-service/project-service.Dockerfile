...
RUN mvn dependency:go-offline -pl project-service -am -q
...
RUN mkdir -p /app/output && \
    cp $(find /app/project-service/target -name "*.jar" ! -name "original-*.jar" | head -n 1) /app/output/app.jar
