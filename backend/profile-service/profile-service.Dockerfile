...
RUN mvn dependency:go-offline -pl profile-service -am -q

COPY . .

RUN mvn clean package -pl profile-service -am -DskipTests

RUN mkdir -p /app/output && \
    cp $(find /app/profile-service/target -name "*.jar" ! -name "original-*.jar" | head -n 1) /app/output/app.jar
